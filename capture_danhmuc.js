...
(async () => {
    ...
    const takeScreenshot = async (filename, action = null) => {
        ...
        if (filename.includes('popup') || filename.includes('confirm') || filename.includes('chitiet') || filename.includes('themsua') || filename.includes('reject') || filename.includes('pheduyet') || filename.includes('thietlap') || filename.includes('timkiem')) {
    ...
    // Quản lý danh mục
    await navigateByPath('Quản lý danh mục', 'Báo cáo danh mục');
    await takeScreenshot('baocao_danhmuc.png');

    await navigateByPath('Quản lý danh mục', 'Danh sách danh mục');
    await takeScreenshot('timkiemnangcao_danhmuc.png', async (p) => {
        const btn = await p.$('button[title="Tìm kiếm nâng cao"]');
        if (btn) await btn.click();
    });
    await closeDialog();

    await takeScreenshot('chitietgiatridanhmuc.png', async (p) => {
        const btn = await p.$('button[title="Chi tiết"]');
        if (btn) await btn.click();
    });
    await closeDialog();

    await takeScreenshot('thietlap_danhmuc.png'); // this might be a main page view
    
    await takeScreenshot('themsua_danhmuc.png', async (p) => {
        const btn = await p.$('button[title="Sửa"]');
        if (btn) await btn.click();
    });
    await closeDialog();

    await takeScreenshot('confirm_delete_cat.png', async (p) => {
        const btn = await p.$('button[title="Xóa"]');
        if (btn) await btn.click();
    });
    await closeDialog();

    await takeScreenshot('confirm_submit_cat.png', async (p) => {
        const btn = await p.$('button[title="Gửi duyệt"]');
        if (btn) await btn.click();
    });
    await closeDialog();

    await navigateByPath('Quản lý danh mục', 'Phê duyệt danh mục');
    await takeScreenshot('pheduyet_danhmuc.png', async (p) => {
        const btn = await p.$('button[title="Phê duyệt"]');
        if (btn) await btn.click();
    });
    await closeDialog();

    await takeScreenshot('reject_cat.png', async (p) => {
