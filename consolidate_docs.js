...
GROUPS.forEach((group, idx) => {
    ...
    const filename = `${String(groupNum).padStart(2, '0')}_${group.title.replace(/ /g, '_')}.md`;
    ...
    group.files.forEach((file, fIdx) => {
        ...
        if (fs.existsSync(filePath)) {
            ...
            // Extract the relevant part (usually after the first # header or similar)
            // But some files might have multiple ## headers if they were already merged.
            // Let's just strip the first # and possibly the first ## if it's the group header.
            const lines = fileContent.split('\n');
            let startLine = 0;
            for (let i = 0; i < lines.length; i++) {
               if (lines[i].startsWith('## ') && (lines[i].includes(group.title) || group.files.length > 1)) {
                  // This is likely the module header. We keep it but fix the numbering.
                  // Wait, if it's a multiple module group, we need to renumber them.
                  // 4.2.3.1.2, 4.2.3.1.3 etc.
                  let moduleNum = group.files.length > 1 ? fIdx + 2 : 1; 
                  lines[i] = `## ${groupCode}.${moduleNum}. ${lines[i].split(' – ')[1] || lines[i].split('. ').slice(1).join('. ')}`;
                  startLine = i;
                  break;
               }
            }
