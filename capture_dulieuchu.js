...
(async () => {
    ...
    const takeScreenshot = async (filename, action = null) => {
        ...
        if (filename.includes('popup') || filename.includes('confirm') || filename.includes('quanlythuoc_tinh') || filename.includes('quytac') || filename.includes('thietlap') || filename.includes('capnhat') || filename.includes('sosanh') || filename.includes('congkhai')) {
    ...
    // Quản lý dữ liệu chủ
    await navigateByPath('Quản lý dữ liệu chủ', 'Quản lý thực thể');
    
    await takeScreenshot('quanlythuoc_tinh.png', async (p) => {
        const btn = await p.$('button[title="Thuộc tính"]');
        if (btn) await btn.click();
    });
    await closeDialog();

    await navigateByPath('Quản lý dữ liệu chủ', 'Quy tắc định danh');
    await takeScreenshot('quytacdinhdanh.png', async (p) => {
        const btn = await p.$('button[title="Quy tắc định danh"]');
        if (btn) await btn.click();
    });
    await closeDialog();

    await navigateByPath('Quản lý dữ liệu chủ', 'Hợp nhất dữ liệu');
    await takeScreenshot('quytachopnhat.png', async (p) => {
        const btn = await p.$('button[title="Quy tắc hợp nhất"]');
        if (btn) await btn.click();
    });
    await closeDialog();

    await takeScreenshot('sosanhhopnhat_master.png', async (p) => {
        const btn = await p.$('button[title="So sánh"]');
        if (btn) await btn.click();
    });
    await closeDialog();

    await navigateByPath('Quản lý dữ liệu chủ', 'Quan hệ thực thể');
    await takeScreenshot('thietlapquanhe.png', async (p) => {
        const btn = await p.$('button[title="Thiết lập quan hệ"]');
        if (btn) await btn.click();
    });
    await closeDialog();

    await navigateByPath('Quản lý dữ liệu chủ', 'Cập nhật dữ liệu');
    await takeScreenshot('capnhatdulieuchu.png', async (p) => {
        const btn = await p.$('button[title="Cập nhật"]');
        if (btn) await btn.click();
    });
    await closeDialog();

    await navigateByPath('Quản lý dữ liệu chủ', 'Công khai dữ liệu');
    await takeScreenshot('congkhaidulieuchu.png', async (p) => {
        const btn = await p.$('button[title="Công khai"]');
        if (btn) await btn.click();
    });
    await closeDialog();
