...
export function ProcessingCooperationPage() {
  const [activeTab, setActiveTab] = useState<'data' | 'processing' | 'warnings' | 'logs'>('data');

  const sampleData = [
    { id: 1, recordId: 'HTQT-2025-001234', projectName: 'Dự án hợp tác với EU', country: 'Liên minh châu Âu', signDate: '15/03/2024', status: 'Đã xử lý' },
    { id: 2, recordId: 'HTQT-2025-001235', projectName: 'Hiệp định tương trợ tư pháp', country: 'Hàn Quốc', signDate: '20/05/2024', status: 'Đang xử lý' },
    { id: 3, recordId: 'HTQT-2025-001236', projectName: 'Điều ước quốc tế về tư pháp', country: 'Nhật Bản', signDate: '10/08/2024', status: 'Đã xử lý' },
  ];

  return (
    <div className="space-y-6">
      <div className="bg-white border border-slate-200 rounded-lg p-6">
        <h2 className="text-xl text-slate-900 mb-2">CSDL Hợp tác quốc tế</h2>
        <p className="text-slate-600">
          Quản lý xử lý dữ liệu từ CSDL Hợp tác quốc tế thuộc Vụ Hợp tác quốc tế.
        </p>
      </div>
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
        ...
        <div className="p-6">
          {activeTab === 'data' && (
            <GenericDataTable
              title="Dữ liệu Hợp tác quốc tế"
              description="Danh sách dữ liệu từ CSDL Hợp tác quốc tế"
              icon={Database}
              iconColor="purple"
              columns={[
                { key: 'recordId', label: 'Mã bản ghi', sortable: true },
                { key: 'projectName', label: 'Tên dự án/Điều ước', sortable: true },
              ...
              searchFields={[
                ...
                { label: 'Tên dự án', name: 'projectName', type: 'text' },
                { label: 'Quốc gia', name: 'country', type: 'text' }
              ]}
              detailFields={[
                { label: 'Mã bản ghi', key: 'recordId' },
                { label: 'Tên dự án/Điều ước', key: 'projectName' },
          ...
          {activeTab === 'processing' && (
            <ProcessingConfigManager 
              ...
              description="Quản lý xử lý dữ liệu từ CSDL Hợp tác quốc tế thuộc Vụ Hợp tác quốc tế."
              dataSourceKey="cooperation"
            />
          )}
              ...
              <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {[
                      { time: '09/12/2024 10:30', type: 'Trùng mã dự án hợp tác', total: 320, success: 310, error: 10, status: 'success' },
                      { time: '09/12/2024 09:15', type: 'Chuẩn hóa tên quốc gia', total: 280, success: 275, error: 5, status: 'success' },
                      { time: '09/12/2024 08:00', type: 'Trùng mã điều ước', total: 250, success: 245, error: 5, status: 'success' },
                      { time: '08/12/2024 16:45', type: 'Chuẩn hóa ngày ký kết', total: 420, success: 400, error: 20, status: 'warning' },
                      { time: '08/12/2024 14:20', type: 'Loại bỏ dữ liệu trùng lặp', total: 360, success: 350, error: 10, status: 'success' }
                    ].map((log, index) => (
