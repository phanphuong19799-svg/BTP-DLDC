...
export function ProcessingBusinessHouseholdPage() {
  const [activeTab, setActiveTab] = useState<'connection' | 'processing' | 'logs'>('connection');

  return (
    <div className="space-y-6">
      <div className="bg-white border border-slate-200 rounded-lg p-6">
        <h2 className="text-xl text-slate-900 mb-2">CSDL Hộ kinh doanh</h2>
        <p className="text-slate-600">
          Quản lý xử lý dữ liệu từ Cơ sở dữ liệu Hộ kinh doanh thuộc Cục Quản lý đăng ký kinh doanh.
        </p>
      </div>
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
        ...
        <div className="p-6">
          ...
          {activeTab === 'processing' && <div className="text-slate-600">Nội dung cấu hình xử lý dữ liệu CSDL Hộ kinh doanh...</div>}
          {activeTab === 'logs' && (
              ...
              <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {[
                      { time: '09/12/2024 10:30', type: 'Trùng mã số hộ KD', total: 950, success: 920, error: 30, status: 'warning' },
                      { time: '09/12/2024 09:15', type: 'Chuẩn hóa địa chỉ kinh doanh', total: 780, success: 770, error: 10, status: 'success' },
                      { time: '09/12/2024 08:00', type: 'Trùng CCCD chủ hộ', total: 650, success: 630, error: 20, status: 'success' },
                      { time: '08/12/2024 16:45', type: 'Chuẩn hóa ngành nghề KD', total: 1120, success: 1100, error: 20, status: 'success' },
                      { time: '08/12/2024 14:20', type: 'Loại bỏ dữ liệu trùng lặp', total: 860, success: 820, error: 40, status: 'warning' }
                    ].map((log, index) => (
