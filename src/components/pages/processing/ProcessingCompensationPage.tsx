...
export function ProcessingCompensationPage() {
  const [activeTab, setActiveTab] = useState<'connection' | 'processing' | 'logs'>('connection');

  return (
    <div className="space-y-6">
      <div className="bg-white border border-slate-200 rounded-lg p-6">
        <h2 className="text-xl text-slate-900 mb-2">CSDL Bồi thường nhà nước</h2>
        <p className="text-slate-600">
          Quản lý xử lý dữ liệu từ Cơ sở dữ liệu Bồi thường nhà nước thuộc Vụ Bồi thường nhà nước.
        </p>
      </div>
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
        ...
        <div className="p-6">
          ...
          {activeTab === 'processing' && <div className="text-slate-600">Nội dung cấu hình xử lý dữ liệu CSDL Bồi thường nhà nước...</div>}
          {activeTab === 'logs' && (
              ...
              <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {[
                      { time: '09/12/2024 10:30', type: 'Trùng mã hồ sơ', total: 420, success: 410, error: 10, status: 'success' },
                      { time: '09/12/2024 09:15', type: 'Chuẩn hóa số tiền bồi thường', total: 380, success: 375, error: 5, status: 'success' },
                      { time: '09/12/2024 08:00', type: 'Trùng CCCD người yêu cầu', total: 350, success: 340, error: 10, status: 'success' },
                      { time: '08/12/2024 16:45', type: 'Chuẩn hóa quyết định bồi thường', total: 520, success: 500, error: 20, status: 'warning' },
                      { time: '08/12/2024 14:20', type: 'Loại bỏ hồ sơ không hợp lệ', total: 460, success: 445, error: 15, status: 'success' }
                    ].map((log, index) => (
