...
export function ProcessingJudgmentPage() {
  const [activeTab, setActiveTab] = useState<'connection' | 'processing' | 'logs'>('connection');

  return (
    <div className="space-y-6">
      <div className="bg-white border border-slate-200 rounded-lg p-6">
        <h2 className="text-xl text-slate-900 mb-2">CSDL Chứng thực</h2>
        <p className="text-slate-600">
          Quản lý xử lý dữ liệu từ Cơ sở dữ liệu Chứng thực thuộc Cục Hành chính tư pháp.
        </p>
      </div>
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
        ...
        <div className="p-6">
          ...
          {activeTab === 'processing' && <div className="text-slate-600">Nội dung cấu hình xử lý dữ liệu CSDL Chứng thực...</div>}
          {activeTab === 'logs' && <div className="text-slate-600">Nội dung lịch sử xử lý CSDL Chứng thực...</div>}
        </div>
      </div>
    </div>
  );
