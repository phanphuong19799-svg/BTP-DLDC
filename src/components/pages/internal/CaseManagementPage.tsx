...
export function CaseManagementPage({ mode = 'thu thập', context = 'thu thập' }: CaseManagementPageProps) {
  ...
  const generateData = () => {
    const data = [
      { id: '1', title: `${mode.charAt(0).toUpperCase() + mode.slice(1)} dữ liệu Nhập Quốc tịch`, icon: UserCheck, color: 'blue', lastMonth: 2098, thisMonth: 3424, change: 63.2 },
      { id: '2', title: `${mode.charAt(0).toUpperCase() + mode.slice(1)} dữ liệu Thôi Quốc tịch`, icon: UserCheck, color: 'green', lastMonth: 3289234, thisMonth: 3424990, change: 4.05 },
      { id: '3', title: `${mode.charAt(0).toUpperCase() + mode.slice(1)} dữ liệu Trở lại Quốc tịch`, icon: UserCheck, color: 'purple', lastMonth: 2995923, thisMonth: 3424878, change: 14.3 },
    ];
  ...
  const chartData = stats.map((stat, index) => {
    const names = ['Nhập QT', 'Thôi QT', 'Trở lại QT'];
    return {
      name: names[index],
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-6">
        ...
        <div className="relative h-96 pl-16 pr-4 pt-8 pb-20">
          ...
          <div className="absolute left-16 right-4 top-8 bottom-20 border-l border-b border-slate-300">
            {[0, 1000000, 2000000, 3000000, 4000000, 5000000, 6000000, 7000000].map((value) => {
          ...
          {/* Bars - Stacked */}
          <div className="absolute left-16 right-4 top-8 bottom-20 flex items-end justify-center gap-16">
            {chartData.map((data, index) => {
              ...
              return (
                <div key={index} className="flex flex-col items-center relative w-20" style={{ height: '100%' }}>
                  <div className="w-full flex flex-col items-center justify-end" style={{ height: '100%' }}>
                    {/* Stacked bar container */}
                    <div className="w-full flex flex-col relative" style={{ height: `${totalPercent}%`, minHeight: '8px' }}>
                  ...
                  {/* Label */}
                  <div className="absolute -bottom-12 left-1/2 transform -translate-x-1/2 whitespace-nowrap text-xs text-slate-600">
                    {data.name}
                  </div>
                </div>
              );
