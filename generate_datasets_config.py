...
# Mapping of ComponentName -> title
sys_names = {
...
def process_file(filepath, comp_name):
    ...
    # extract items
    items = []
    # match name: '...' or name: "..." inside tableData or similar
    # let's be safe and extract from tableData
    td_match = re.search(r'const tableData.*?(?=\s*export|\s*const|\s*return)', content, re.DOTALL)
    if td_match:
        items = re.findall(r"name:\s*['\"]([^'\"]+)['\"]", td_match.group(0))
    else:
        # fallback to all name matches
...
for dir_path in [internal_dir, set('internal', 'external')]: # wait...
    pass
...
print("Generated src/components/pages/processing/datasets_config.ts successfully!")
