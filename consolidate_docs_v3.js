...
GROUPS.forEach((group, idx) => {
    ...
    // 1. Dashboard Section (stays as .1)
    ...
    // 2. Modules Sections (start from .2)
    group.files.forEach((file, fIdx) => {
        ...
        if (fs.existsSync(filePath)) {
            ...
            let processedLines = lines.map(line => {
                const headerMatch = line.match(/^(#{2,5}) ([\d.]+)(.*)/);
                if (headerMatch) {
                    const hashes = headerMatch[1];
                    const oldNum = headerMatch[2];
                    const titleRest = headerMatch[3];
                    
                    // Old header level: ## = x.x.x.x, ### = x.x.x.x.y, #### = x.x.x.x.y.z
                    // Actually, let's just count dots in oldNum
                    const oldParts = oldNum.split('.').filter(p => p !== '');
                    const suffix = oldParts.slice(4).join('.'); // Get everything after 4.2.3.1
                    
                    if (hashes === '##') {
                        return `### ${moduleCode}${titleRest}`;
                    } else if (hashes === '###') {
                        const z = oldParts[oldParts.length - 1];
                        return `#### ${moduleCode}.${z}${titleRest}`;
                    } else if (hashes === '####') {
                        // Sub-sub headings like 4.2.3.1.1.1
                        const z = oldParts[oldParts.length - 2];
                        const w = oldParts[oldParts.length - 1];
                        return `##### ${moduleCode}.${z}.${w}${titleRest}`;
                    }
                }
                return line;
            });
